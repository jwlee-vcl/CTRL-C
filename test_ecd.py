...
from datasets import build_ecd_dataset
...
def compute_hl_ecd_np(hl, sz, eps=1e-6):
    (a,b,c) = hl
    if b < 0:
        a, b, c = -a, -b, -c
    b = np.maximum(b, eps)
    ...
    x = 1
    left = np.array([0 - sx, (-a*x - c)/b - sy])        
    x = sz[1]
    right = np.array([sz[1]-1 - sx, (-a*sz[1] - c)/b - sy])
...
def main(cfg):
    ...
    dataset_test = build_ecd_dataset(image_set='test', cfg=cfg)
    ...
    with open(csvpath, 'w', newline='') as csvfile:
        ...
        csvwriter.writerow(['filename', 'hl-err'])
    ...
    for i, (samples, extra_samples, targets) in enumerate(tqdm(data_loader_test)):
        with torch.no_grad():
            ...
            target_hl_pts = compute_hl_ecd_np(target_hl, img_sz)
            pred_hl_pts = compute_hl_np(pred_hl, img_sz)
            ...
            with open(csvpath, 'a', newline='') as csvfile:
                csvwriter = csv.writer(csvfile)
                csvwriter.writerow([filename, err_hl])
