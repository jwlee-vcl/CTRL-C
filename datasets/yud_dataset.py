...
class YUDDataset(Dataset):
    def __init__(self, cfg, basepath, return_masks=False, transform=None):        
        ...
        for filename in os.listdir(basepath):
            if '.jpg' in filename:
                ...
                self.list_img_filename.append(osp.join(self.basepath,filename))
    ...
    def __getitem__(self, idx):
        ...
        dat_filename = img_filename.replace('.jpg', '.mat')
        ...
        # gt data
        mat = sio.loadmat(dat_filename)    
        gt_vps = mat['vp_orthogonal']
        gt_vps[1] = -gt_vps[1]
        
        gt_focal = 674.9180
        gt_fovy = 2.0*np.arctan2(240.0, gt_focal)
        gt_focal /= 240.0
        
        gt_up = gt_vps[:,1]
        
        gt_hl = gt_up.copy()
        gt_hl[2] = gt_hl[2]*(gt_focal)       
        ...
        target['up_vector'] = torch.from_numpy(np.ascontiguousarray(gt_up)).contiguous().float()
...
def build_yud(image_set, cfg):
    ...
    root = '/data/project/rw/yud_gt/'

    img_folder = root
    dataset = YUDDataset(cfg, img_folder, return_masks=cfg.MODELS.MASKS, transform=make_transform())
    return dataset