...
def main(cfg):
    ...
    dataset_test = build_yud_dataset(image_set='test', cfg=cfg)
    ...
    with open(csvpath, 'w', newline='') as csvfile:
        ...
        csvwriter.writerow(['filename', 'angle-err', 'pitch-err', 'roll-err', 'fov-err', 'hl-err'])
    ...
    for i, (samples, extra_samples, targets) in enumerate(tqdm(data_loader_test)):
        with torch.no_grad():
            ...
            with open(csvpath, 'a', newline='') as csvfile:
                csvwriter = csv.writer(csvfile)
                csvwriter.writerow([filename, err_angle, err_pitch, err_roll, err_fovy, err_hl])
