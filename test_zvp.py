...
def compute_zvp_error(pred_zvp, target_zvp):    
    cos_sim = cosine_similarity(target_zvp, pred_zvp)
    err_angle = np.degrees(np.arccos(np.clip(np.abs(cos_sim),0.0, 1.0)))    
    return err_angle    
...
def main(cfg):
    ...
    with open(csvpath, 'w', newline='') as csvfile:
        ...
        csvwriter.writerow(['filename', 'angle-err'])
    ...
    for i, (samples, extra_samples, targets) in enumerate(tqdm(data_loader_test)):
        with torch.no_grad():
            ...
            err_angle = compute_zvp_error(pred_zvp, target_zvp)
                        
            with open(csvpath, 'a', newline='') as csvfile:
                csvwriter = csv.writer(csvfile)
                csvwriter.writerow([filename, err_angle])
